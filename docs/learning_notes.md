...
## Dockerfile の　PYTHONBUFFERED=1について
バッファーを入れるか入れないか   
1 -> 出力が発生したらすぐに書き出される = ログがリアルタイムでコンソールに表示されるのでデバッグしやすい   

なので大量のログ出力が期待される場合などはこの設定はつけない方がいい　　
例) データ処理やETLとか.   

なので, 勉強が終わったら消しておく

## エントリーポイントについて
あるコンテナが起動したら必ず実行するメインのコマンドのこと

`ENTRYPOINT ["dbt", "run"]` とすれば `docker run <image>` が引き金となってコンテナ内では `dbt run` が実行される

`CMD []` は `ENTRYPOINT` に渡すデフォルトの引数であったり，`ENTRYPOINT` がない場合の代わりの実行コマンド